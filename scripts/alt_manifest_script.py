    ...
    for root, dirs, files in os.walk(dir_name, followlinks=True):
        ...
        for _x in files:
            ...
            if re.search(r"(?<=FLD\d{4}).\w*.fastq.gz$", _x) is not None:
                ...
                sample_name = re.search(r'*?_R\d', _x).group()
                sample_name = sample_name.replace('_R\d','')
                sample_name = sample_name.replace('_', '')
                cwd = os.getcwd()
                abs_path = cwd + '/' + os.path.join(root, _x)
                direction = re.search(r'R\d', _x)
                if direction.group() == "R1":
                    ...
                    temp = "reverse"
                csv_list.append([sample_name, abs_path, temp])
            elif re.search(r'_R\d', _x) is not None:
                sample_name = re.search(r'.*_R', _x).group()
                sample_name = sample_name.replace('_R', '')
                sample_name = sample_name.replace('_', '')
                cwd = os.getcwd()
                abs_path = cwd + '/' + os.path.join(root, _x)
                direction = re.search(r'R\d', _x)
